...
$id=$_REQUEST['id'];
$sql="select * from article,article_cate where article.cateid=article_cate.cateid and article.cateid={$id}";
...
$sql="select * from article,article_cate where article.cateid=article_cate.cateid and article.cateid={$id} order by artiid desc limit {$offset},{$pageSize}";
$rows=fetchAll($sql);
...
<body><?php include_once("baidu_js_push.php") ?>
	...
	<div id="content" class="container">
		<div id="location">
				当前位置：<a href="index.php">首页</a>&nbsp;&gt;&nbsp;<a href="essay.php">技术杂谈</a>&nbsp;&gt;&nbsp;<?php $sql="select * from article_cate where cateid={$id}";$row=fetchOne($sql);echo $row['catename'];?>
		</div>
		<article class="left">
			<h3>文章推荐</h3>
